...
class Astar2D:
    ...
    def run(self, start, goal, graph):
        ...
        if len(path) == 0:
            ...
            for i in range(len(path)):
                nodes.append(path[i].global_point)

            for j in range(len(obstacles)):
                walls.append(obstacles[j].global_point)
            ...
            for i in range(0, np.size(graph.cells, 0)):
                for j in range(0, np.size(graph.cells, 1)):
                    if graph.cells[i][j].occupied:
                        allobstacles.append(
                            (graph.cells[i][j].x + graph.origin[0], graph.cells[i][j].y + graph.origin[1]))
                    else:
                        allpoints.append((graph.cells[i][j].x + graph.origin[0], graph.cells[i][j].y + graph.origin[1]))
    ...
    def find_path(self, start, goal, graph):
        ...
        for i in range(0, np.size(grid.cells, 0)):
            for j in range(0, np.size(grid.cells, 1)):
                if float(nodes[i][j].global_point[0]) == float(start[0]) and float(nodes[i][j].global_point[1]) == float(start[1]):
                    start_index = (i,j)
                elif float(nodes[i][j].global_point[0]) == float(goal[0]) and float(nodes[i][j].global_point[1]) == float(goal[1]):
                    goal_index = (i,j)
        if start_index is None:
            ValueError('Start Point could not be found in graph!')
        elif goal_index is None:
            ValueError('Goal Point could not be found in graph')
        else:
